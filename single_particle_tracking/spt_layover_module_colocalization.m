...
axis off
    ...
    function colocalization_callback(~,~,~)
        ...
        if isempty(input_values)==1
            ...
            if ~isempty(tracks)
                [selected,to_plot] = spt_layover_module_colocalization_inside(tracks,image,pixel_size,percentage_value);
                plot_all()   
...
function [localizaed_tracks,to_plot] = spt_layover_module_colocalization_inside(tracks,image,pixel_size,percentage_value)
...
for m = 1:size(counts,1)
    if counts(m,1)>=(sum(counts(m,:))*percentage_value/100)
        localizaed_tracks = [localizaed_tracks,m];        
    end
end

tracks_colocalized = tracks(localizaed_tracks);
tracks_not_colocalized = tracks(setdiff(1:length(tracks),localizaed_tracks));
